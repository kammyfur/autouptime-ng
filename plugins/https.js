module.exports = function (settings) {
	...
	processor = function (settings) {
		console.log("au-https > Requesting " + 'https://' + settings.Host + ":" + settings.Port + "/" + settings.Root + "...");
		AULib.HttpRequest = require("sync-request");
		try {
			var res = AULib.HttpRequest('GET', 'https://' + settings.Host + ":" + settings.Port + "/" + settings.Root, {headers: {'user-agent': '+MinteckProjectsAutoUptime/' + AUBuildProperties.version,},});
		} catch (e) {
			console.log(e.stack);
			outputCode = 3;
		...
		console.log("au-https > Status code for " + 'https://' + settings.Host + ":" + settings.Port + "/" + settings.Root + " is " + res.statusCode);
